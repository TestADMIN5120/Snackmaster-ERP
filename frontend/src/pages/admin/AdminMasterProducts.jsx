  ...
  async function handleAdd(e) {
    ...
    try {
      await addDoc(collection(db, "master_products"), {
        ...
        gstRatePercent: Number(gstRate) || 0,
        costWithGst: Number(costWithGst) || 0,
        costWithoutGst: Number(costWithoutGst) || 0,
        orgId: orgId,
      ...
      <div style={{ ...card, marginBottom: 30 }}>
        ...
        <form onSubmit={handleAdd}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 15, marginBottom: 20 }}>
            ...
            <label style={label}>Selling Price (MRP) ₹ <input type="number" step="0.01" value={mrp} onChange={e=>setMrp(e.target.value)} style={input} placeholder="20.00" /></label>
            <label style={label}>Cost Price (+GST) ₹ <input type="number" step="0.01" value={costWithGst} onChange={e=>setCostWithGst(e.target.value)} style={input} placeholder="15.00" /></label>
            <label style={label}>Cost Price (W/O GST) ₹ <input type="number" step="0.01" value={costWithoutGst} onChange={e=>setCostWithoutGst(e.target.value)} style={input} placeholder="13.50" /></label>
            <label style={label}>GST Rate (%) <input type="number" step="0.01" value={gstRate} onChange={e=>setGstRate(e.target.value)} style={input} placeholder="12" /></label>
            <label style={label}>Margin (%) <input type="number" step="0.01" value={margin} onChange={e=>setMargin(e.target.value)} style={input} placeholder="25" /></label>
              ...
              <tbody>
                {masterList.map(p => (
                  <tr key={p.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    <td style={{...td, fontFamily: "monospace", color: "#0284c7"}}>{p.sku}</td>
                    ...
                    <td style={td}>{p.marginPercent}%</td>
                    <td style={td}>
