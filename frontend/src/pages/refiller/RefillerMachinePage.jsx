...
export default function RefillerMachinePage() {
  ...
  async function handleAcceptKit() {
    if (!window.confirm("Accept this kit? Items will be deducted from warehouse.")) return;
    setSaving(true);
    try {
        ...
        await batch.commit();
        alert("✅ Kit Accepted!");
    } catch (err) { alert("Failed to accept kit."); } finally { setSaving(false); }
  }
  ...
  async function completeRefill() {
    ...
    try {
      ...
      // 🟢 API Call updated to use the production URL
    ...
    } catch (err) { alert("Error confirming refill. Ensure backend is live."); } 
...
const inputSmall = { width: "80%", padding: "8px", borderRadius: 6, border: "1px solid #3b82f6", textAlign: 'center', fontWeight: 'bold', fontSize: 18, outline: 'none' };
const prodText = { fontSize: 11, color: '#475569', margin: '4px 0', height: 28, overflow:'hidden' };
